...
SELECT DNO, SALARY FROM EMPLOYEE
```
Exibe todas as combinações de departamento incluindo as linhas repetidas. 
```{sql connection=con}
SELECT DISTINCT DNO, SALARY FROM EMPLOYEE
```
Remove as linhas repetidas quando o par `DNO` e `SALARY` são iguais
```{sql connection=con}
SELECT DISTINCT DNO, SALARY FROM EMPLOYEE ORDER BY DNO, SALARY
```
Remove as linhas repetidas quando os pares são iguais e ordenapor departamento e dentro de cada departamento, pelo salário.
...
```
Sim, pois a aplicação da lei distributiva do operador `AND` sobre `OR`, o `AND`vem antes do `OR`
...
```
SIM, pois é outra forma da lei distributiva.
...
```
a 2º não é equivalente a 1º pois a segunda usa `AND`logo é mais restritiva enquanto a 3º é equivalente a 1º pois é mais abrangente. 
...
WHERE NOT ACRES < 50 AND NOT STATE = 'MA'
```
Aqui a 3º é equivalente enquanto a 2º não, pois de novo segue a ordem de prioridade entre `AND` e o `OR`  
