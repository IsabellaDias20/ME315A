...
Apresente o nome da reserva, a área atual e a nova área após o aumento.
```{sql connection=con}
...
```{sql connection=con}
SELECT AVG(ACRES) AS MEDIO, MAX(ACRES) AS MAXIMO, MIN(ACRES) AS MINIMO FROM PRESERVE
```
...
```{sql connection=con}
SELECT COUNT(PNAME) AS NOMES_TOTAIS, COUNT(DISTINCT PNAME) AS NOMES_DISTINTOS FROM PRESERVE
```
...
```{sql connection=con}
SELECT SUM(SALARY) AS SOMA, AVG(SALARY) AS MEDIA, MAX(SALARY) AS VALOR_MAXIMO, MIN(SALARY) AS VALOR_MINIMO FROM EMPLOYEE
```

12.  Quantos funcionários trabalham no Departamento 20?
```{sql connection=con}
SELECT COUNT(*) AS QT_FUNCIONARIOS FROM EMPLOYEE
WHERE DNO = "20"
```

13.  Quantos departamentos possuem funcionários?
```{sql connection=con}
SELECT COUNT(DISTINCT DNO) AS QT_FUNCIONARIOS FROM EMPLOYEE
```
